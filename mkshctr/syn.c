...
static int symbol;			/* cyylex value */
static int sALIAS = ALIAS;		/* 0 in yyrecursive */

#define REJECT		(reject = true)
#define ACCEPT		(reject = false)
#define token(cf)	((reject) ? (ACCEPT, symbol) : (symbol = cyylex(cf)))
#define tpeek(cf)	((reject) ? (symbol) : (REJECT, symbol = cyylex(cf)))
...
/*
 * Helper function called from within lex.c:cyylex() to parse
